 ...
 * and Loop will find and pause the corresponding subscription.
 */
...
export async function POST(
  ...
  console.log(`[Pause] Input: ${subscriptionId} -> Loop format: ${loopSubscriptionId}`);

  try {
    const response = await fetch(`${LOOP_API_BASE}/subscription/${loopSubscriptionId}/pause`, {
      method: 'POST',
      headers: {
    ...
    console.log(`[Pause] Loop API response:`, { status: response.status, data });

    if (!response.ok) {
      return NextResponse.json({
        success: false,
        error: data.message || 'Failed to pause subscription in Loop',
        loopResponse: data,
        loopSubscriptionId,
