...
import { FormulaId, PackSize, PurchaseType, ProtocolId, ProtocolTier } from './productData';
...
export const FORMULA_VARIANTS: Record<FormulaId, Record<FormulaPackSize, string>> = {
...
export const TRIAL_PACK_VARIANTS: Record<FormulaId, Record<TrialPackSize, string>> = {
...
export function getFormulaVariantId(
  ...
  const variantId = FORMULA_VARIANTS[formulaId]?.[packSize as keyof typeof FORMULA_VARIANTS['01']];
  if (!variantId) return null;
  
  if (purchaseType === 'subscription') {
    const sellingPlanId = FORMULA_SELLING_PLANS[packSize as keyof typeof FORMULA_SELLING_PLANS];
...
export function getUnconfiguredVariants(): string[] {
  ...
  for (const formulaId of ['01', '02'] as FormulaId[]) {
    for (const packSize of ['4', '8', '12', '28'] as PackSize[]) {
      for (const purchaseType of ['subscription', 'one-time'] as PurchaseType[]) {
        const variantData = getFormulaVariantId(formulaId, packSize, purchaseType);
        if (!isVariantConfigured(variantData?.variantId || null)) {
          unconfigured.push(`Formula ${formulaId} - ${packSize}-pack - ${purchaseType}`);
        }
      }
    }
  }
  ...
  for (const protocolId of ['1', '2', '3', '4'] as ProtocolId[]) {
    const tiers = protocolId === '4' 
      ? ['pro', 'max'] as ProtocolTier[]
      : ['starter', 'pro', 'max'] as ProtocolTier[];
    
    for (const tier of tiers) {
      for (const purchaseType of ['subscription', 'one-time'] as PurchaseType[]) {
        const variantData = getProtocolVariantId(protocolId, tier, purchaseType);
        if (!isVariantConfigured(variantData?.variantId || null)) {
          unconfigured.push(`Protocol ${protocolId} - ${tier} - ${purchaseType}`);
        }
      }
    }
  }
