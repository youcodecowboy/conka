...
function StatBar({ stat, index }: { stat: ImprovementStat; index: number }) {
  ...
  return (
    <div 
      className="neo-box p-4"
      style={{ animationDelay: `${index * 100}ms` }}
    >
      <div className="flex items-start justify-between mb-2">
        <div>
          <p className="font-clinical text-sm font-medium">{stat.metric}</p>
          {stat.description && (
            <p className="font-clinical text-xs opacity-60 mt-0.5">{stat.description}</p>
          )}
        </div>
      ...
      <div className="flex justify-between mt-1">
        <span className="font-clinical text-xs opacity-50">0%</span>
...
function ComparisonChart({ athlete }: { athlete: AthleteData }) {
  ...
  return (
    <div className="space-y-3">
        ...
        return (
          <div key={metric} className="space-y-1">
            <div className="flex justify-between items-center">
              <span className="font-clinical text-xs opacity-70">{formatMetricName(metric)}</span>
      ...
      {/* Legend */}
      <div className="flex justify-center gap-6 pt-2 border-t border-current/10">
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 rounded bg-current/30" />
          <span className="font-clinical text-xs opacity-70">Baseline</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 rounded bg-emerald-500" />
          <span className="font-clinical text-xs opacity-70">Results</span>
        </div>
      </div>
    </div>
  );
...
export default function AthleteStats({ athlete, compact = false }: AthleteStatsProps) {
  if (compact) {
    // Compact view for mobile or sidebar previews
    return (
      <div className="grid grid-cols-2 gap-2">
        {athlete.improvements.slice(0, 4).map((stat, idx) => (
          <div key={stat.metric} className="text-center p-2 neo-box">
            <p className="text-lg font-bold font-clinical">{stat.value}</p>
            <p className="font-clinical text-xs opacity-70 truncate">{stat.metric}</p>
          </div>
        ))}
      </div>
    );
