...
| Hero assets | ADHD `/lifestyle/GirlsLaughing.jpg` (1500x1500). Productivity and brain-ageing swapped to real product shots on 2026-09-09: `/lifestyle/flow/TrackAthlete.webp` and `/lifestyle/ageing/BottleBalance.webp` (both 928x1152 portrait). | ADHD is still a stand-in. The two new ones are portrait in a 3:2 frame, so each config sets an `objectPosition` chosen against a rendered crop rather than by eye. `WorkingWoman.jpg` was deleted with the swap; the old `FlowDrink.jpg` hero reference is retired; `FlowDrink.jpg` is still used by `general-listicle.ts`. |
