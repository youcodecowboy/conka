...
4. **A mobile pass followed the first review, after visual comparison against the two reference sites.** The plan's "fixed aspect ratio on mobile" and "one tuned `object-position` on desktop" mitigation was replaced by something truer to Gray Matter and Cadence: every image now runs **full bleed and square-cornered below 768px**, and the hero **inverts on mobile** so the photo leads flush under the nav with the copy beneath. The `object-position` was dropped entirely, because the hero frame is square at every breakpoint so only the horizontal axis ever overflows and a vertical percentage could never apply. The hero stat row is centred per column; three uneven left-aligned blocks read as ragged at 390px where the labels wrap to different line counts.

   Images were then also pulled **flush to the top of their section** on mobile, so no band of section background shows above the picture. A chapter image cancels the 5rem `--brand-section-padding-mobile` with `-mt-20`; the hero cancels `brand-hero-first`'s 1rem with `-mt-4`. Bottom padding is deliberately left alone: only the top was doubling up against the previous section's breathing room.

   Three cascade couplings this created, all commented at the call site: `-mx-5` cancels `--brand-gutter-mobile` (1.25rem), `-mt-20` cancels `--brand-section-padding-mobile` (5rem), and `-mt-4` cancels `--brand-space-m` (1rem). None could be expressed as a Tailwind `pt-0`/`px-0`, because `.brand-section` is unlayered and would win. The cancellations are exact because Tailwind's `--spacing` is `0.25rem`; they were verified against the generated CSS, not assumed. **If the section-padding or gutter tokens change, these negative margins must change with them.** The reset breakpoint is `md:` (768px), which is where the gutters actually change, not `lg:`.
