...
**Mobile treatment (deliberate, benchmarked against Gray Matter and Cadence).** Every image runs **full bleed and square-cornered below 768px**, then insets to `rounded-md` from 768px where the section gutters return. Each image also sits **flush with the top of its section**: a chapter image cancels the 5rem `--brand-section-padding-mobile`, and the hero cancels `brand-hero-first`'s 1rem, so no band of section background shows above the picture. Bottom padding is untouched; only the top was doubling up against the previous section's breathing room. The hero inverts on mobile: the photo leads, flush under the nav, with the copy beneath it, while desktop keeps copy left and photo right. Copy still leads in the DOM so the `h1` precedes the image for screen readers and crawlers; only the visual order flips.

> These cancellations are hard-coded negative margins (`-mt-20`, `-mt-4`, `-mx-5`) rather than Tailwind `pt-0`/`px-0`, because `.brand-section` is unlayered CSS and would win. They are exact: Tailwind's `--spacing` is `0.25rem`, so `-mt-20` is 5rem and `-mx-5` is 1.25rem. If the section padding or gutter tokens ever change, these must change with them.
